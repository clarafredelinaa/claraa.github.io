    ...
    return declare([BaseWidgetSetting, _WidgetsInTemplateMixin], {
      ...
      addQueryFields: function () {
        ...
        array.forEach(this.layersTable.getRows(), function (row) {
          ...
          var allFields = layer.fields;
          //var fields = array.filter(allfields, function (flds) {
          //  return flds.editable === true;
          //})
          array.forEach(allFields, function (fld) {
            fld["fieldName"] = fld.name;
            fld["isEditable"] = fld.editable;
            fld["label"] = fld.alias ? fld.alias : fld.name;
          })
          var fields = this.formatFieldsFOrDropDown(allFields);
      ...
      formatFieldsFOrDropDown: function (fields) {
        var result = [{
          label: this.nls.page2.layerTable.queryFieldDefaultOption,
          value: 'NOTSET1'
        }];
      ...
      loadFieldsTable: function () {
        ...
        array.forEach(this.layersTable.getRows(), function (row) {
          ...
          if (rowData.update === true) {
            ...
            var allfields = layer.fields;
            var allfieldsInfos = array.filter(allfields, function (flds) {
              return flds.editable === true;
            })
            array.forEach(allfieldsInfos, function (afi) {
              var exists = fields.some(function (field) { return field.fieldName === afi.name });
              if (!exists) {
                var missingField = {}
                missingField["fieldName"] = afi.name;
                missingField["isEditable"] = afi.editable;
                missingField["label"] = afi.alias ? afi.alias : afi.name;
